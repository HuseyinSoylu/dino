...
export const publishArticle = async (
  ...
  try {
    ...
    const transaction = await contract.publishArticle(
      articleHash,
      priceInWei,
      {
        value: publishingFee,
...
export const getLatestArticleNumberFromContract = async () => {
  try {
    ...
    const latestArticleNumber =
      await contract.getLatestArticleNumber();
...
export const purchaseArticle = async (tokenId, price) => {
  try {
    ...
    // ethers.js ile işlem yaparken kullanmak üzere hesap (account) nesnesini oluşturun
    const signer = provider.getSigner();
    ...
    const tx = await contract.purchaseArticle(tokenId, price, {
      value: 1000000000000000, // Send the specified price as Ether
    });
